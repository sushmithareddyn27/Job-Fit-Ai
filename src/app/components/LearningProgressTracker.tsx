...
export function LearningProgressTracker({ courses }: LearningProgressTrackerProps) {
  if (courses.length === 0) {
    return (
      ...
      {courses.map((course) => (
        <div key={course.courseId} className="p-4 border rounded-lg">
          <div className="flex items-start justify-between mb-3">
            ...
            >
              {course.status === 'completed' && <CheckCircle2 className="w-3 h-3 mr-1" />}
              {course.status === 'in-progress' && <PlayCircle className="w-3 h-3 mr-1" />}
              {course.status === 'not-started' && <Clock className="w-3 h-3 mr-1" />}
